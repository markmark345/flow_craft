...
func (h *Hub) Run() {
	...
	for {
		...
		case client := <-h.register:
			...
			h.mu.Unlock()
			h.logger.Debug().Str("remote_addr", client.conn.RemoteAddr().String()).Msg("client connected")
		case client := <-h.unregister:
			h.mu.Lock()
			if _, ok := h.clients[client]; ok {
				delete(h.clients, client)
				client.conn.Close()
				h.logger.Debug().Str("remote_addr", client.conn.RemoteAddr().String()).Msg("client disconnected")
			}
			h.mu.Unlock()
