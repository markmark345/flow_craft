...
func (h *NodeTestHandler) test(c *gin.Context) {
	...
	default:
		utils.JSONError(c, http.StatusBadRequest, apierrors.ErrBadRequest, "invalid kind", nil)
		return
	}
...
func (h *NodeTestHandler) testAppAction(c *gin.Context, user domain.AuthUser, req nodeTestRequest) nodeTestResult {
	...
	case "gmail":
		...
		credProvider, payload, err := h.loadCredentialPayload(c.Request.Context(), user, req.CredentialID)
		if err != nil {
		...
		accessToken, err := h.googleAccessToken(c.Request.Context(), payload)
		if err != nil {
			return nodeTestResult{Success: false, Message: err.Error()}
		}
		profile, err := google.GetProfile(c.Request.Context(), accessToken)
		if err != nil {
			return nodeTestResult{Success: false, Message: err.Error()}
		}
	...
	case "googleSheets":
		...
		sheetID, err := google.ResolveSheetID(c.Request.Context(), accessToken, spreadsheetID, sheetName)
		if err != nil {
			return nodeTestResult{Success: false, Message: err.Error()}
		}
	...
	case "github":
		...
		repoInfo, err := github.GetRepo(c.Request.Context(), accessToken, owner, repo)
		if err != nil {
			return nodeTestResult{Success: false, Message: err.Error()}
		}
	...
	case "bannerbear":
		...
		if templateUID != "" {
			out, _, err := bannerbear.GetTemplate(c.Request.Context(), apiKey, templateUID)
			if err != nil {
				return nodeTestResult{Success: false, Message: err.Error()}
			}
			return nodeTestResult{Success: true, Message: "Connected to Bannerbear", Preview: out}
		}
		out, _, err := bannerbear.ListTemplates(c.Request.Context(), apiKey, 1, 1)
		if err != nil {
			return nodeTestResult{Success: false, Message: err.Error()}
		}
		return nodeTestResult{Success: true, Message: "Connected to Bannerbear", Preview: out}
...
func (h *NodeTestHandler) testAgentModel(c *gin.Context, user domain.AuthUser, req nodeTestRequest) nodeTestResult {
	...
	case "openai", "grok", "custom":
		...
		raw, status, err = openai.ChatCompletions(c.Request.Context(), baseURL, apiKey, payload)
	case "gemini":
		payload := map[string]any{
			"contents": []any{
				map[string]any{
		...
		raw, status, err = gemini.GenerateContent(c.Request.Context(), baseURL, apiKey, model, payload)
	}
...
func (h *NodeTestHandler) loadCredentialPayload(ctx context.Context, user domain.AuthUser, credentialID string) (string, map[string]any, error) {
	if h.creds == nil {
...
func (h *NodeTestHandler) googleAccessToken(ctx context.Context, payload map[string]any) (string, error) {
