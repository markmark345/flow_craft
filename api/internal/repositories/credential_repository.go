...
func (r *CredentialRepository) Create(ctx context.Context, cred entities.Credential) error {
	var projectID any
	if cred.ProjectID != "" {
...
func (r *CredentialRepository) ListForUser(ctx context.Context, userID string) ([]entities.Credential, error) {
...
func (r *CredentialRepository) ListForProject(ctx context.Context, projectID string) ([]entities.Credential, error) {
	rows, err := r.db.QueryContext(ctx, `
        SELECT id, user_id, project_id, provider, name, data_encrypted, created_at, updated_at
        FROM credentials
        WHERE project_id = $1
...
func (r *CredentialRepository) Get(ctx context.Context, id string) (*entities.Credential, error) {
...
func (r *CredentialRepository) Update(ctx context.Context, cred entities.Credential) error {
	var projectID any
	if cred.ProjectID != "" {
		projectID = cred.ProjectID
	}
...
func scanCredentials(rows *sql.Rows) ([]entities.Credential, error) {
	var items []entities.Credential
	for rows.Next() {
		var cred entities.Credential
		var projectID sql.NullString
		var createdAt time.Time
