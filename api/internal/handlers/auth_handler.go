...
type AuthHandler struct {
	...
	oauthAccounts *repositories.OAuthAccountRepository
...
func (h *AuthHandler) forgotPassword(c *gin.Context) {
	...
	if err := h.auth.RequestPasswordReset(c, req.Email, lang); err != nil {
...
func (h *AuthHandler) resetPassword(c *gin.Context) {
	...
	if err := h.auth.ResetPassword(c, req.Token, req.NewPassword); err != nil {
		c.JSON(http.StatusBadRequest, dto.ResponseEnvelope{Error: &dto.ErrorBody{Code: "bad_request", Message: err.Error()}})
		return
	}
	c.JSON(http.StatusOK, dto.ResponseEnvelope{Data: gin.H{"ok": true}})
...
func (h *AuthHandler) oauthCallback(c *gin.Context) {
	...
	if code == "" || state == "" {
		c.JSON(http.StatusBadRequest, dto.ResponseEnvelope{Error: &dto.ErrorBody{Code: "bad_request", Message: "missing code or state"}})
		return
	}
	stateSecret := []byte(h.cfg.OAuthStateSecret)
	if len(stateSecret) == 0 {
...
func (h *AuthHandler) handleLoginOAuthCallback(c *gin.Context, provider string, code string, payload utils.OAuthStatePayload) {
	switch provider {
	case "google":
		token, err := google.ExchangeCode(c, h.cfg.GoogleClientID, h.cfg.GoogleClientSecret, h.cfg.GoogleRedirectURL, code)
		if err != nil {
			c.JSON(http.StatusBadRequest, dto.ResponseEnvelope{Error: &dto.ErrorBody{Code: "bad_request", Message: err.Error()}})
			return
		}
		profile, err := google.FetchUserProfile(c, token.AccessToken)
		if err != nil {
			c.JSON(http.StatusBadRequest, dto.ResponseEnvelope{Error: &dto.ErrorBody{Code: "bad_request", Message: err.Error()}})
			return
		}
	...
	case "github":
		token, err := github.ExchangeCode(c, h.cfg.GitHubClientID, h.cfg.GitHubClientSecret, h.cfg.GitHubRedirectURL, code)
		if err != nil {
			c.JSON(http.StatusBadRequest, dto.ResponseEnvelope{Error: &dto.ErrorBody{Code: "bad_request", Message: err.Error()}})
			return
		}
		profile, err := github.FetchUserProfile(c, token.AccessToken)
		if err != nil {
			c.JSON(http.StatusBadRequest, dto.ResponseEnvelope{Error: &dto.ErrorBody{Code: "bad_request", Message: err.Error()}})
			return
		}
		if profile.Email == "" {
			c.JSON(http.StatusBadRequest, dto.ResponseEnvelope{Error: &dto.ErrorBody{Code: "bad_request", Message: "github email not available"}})
			return
		}
...
func (h *AuthHandler) handleCredentialOAuthCallback(c *gin.Context, provider string, code string, payload utils.OAuthStatePayload) {
	...
	if userID == "" {
		c.JSON(http.StatusBadRequest, dto.ResponseEnvelope{Error: &dto.ErrorBody{Code: "bad_request", Message: "missing user context"}})
		return
	}
	user, err := h.users.Get(c, userID)
	if err != nil {
		c.JSON(http.StatusBadRequest, dto.ResponseEnvelope{Error: &dto.ErrorBody{Code: "bad_request", Message: "invalid user"}})
		return
	}
	scope := payload.Scope
	...
	case "google":
		...
		created, err := h.creds.Create(c, services.AuthUser{ID: user.ID}, scope, projectID, cred)
		if err != nil {
			h.redirectWithError(c, payload.ReturnTo, err.Error())
			return
		}
...
func (h *AuthHandler) findOrCreateOAuthUser(ctx *gin.Context, provider string, providerUserID string, email string, name string, username string) (*entities.User, error) {
	if providerUserID != "" {
...
func (h *AuthHandler) redirectWithSession(c *gin.Context, user *entities.User, next string) {
	token, err := h.auth.CreateSession(c, user.ID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, dto.ResponseEnvelope{Error: &dto.ErrorBody{Code: "internal", Message: err.Error()}})
		return
	}
